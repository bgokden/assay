...
from assay.checkpoint import remove_checkpoint, state_bytes, write_checkpoint
...
def test_a_short_write_is_discarded(tmp_path, monkeypatch):
    ...
    def short(state, target):
        with open(target, "wb") as f:
            f.write(b"x" * 8)

    monkeypatch.setattr("assay.checkpoint.torch.save", short)
    assert not write_checkpoint(path, {"model": torch.ones(1000), "step": 2}, 2)
    assert torch.load(path, weights_only=False)["step"] == 1


def test_a_finished_run_drops_its_checkpoint(tmp_path):
    path = str(tmp_path / "checkpoint.pt")
    assert write_checkpoint(path, {"model": torch.ones(4), "step": 1}, 1)
    with open(path + ".tmp", "wb"):
        pass
    remove_checkpoint(path)
    assert not os.path.exists(path) and not os.path.exists(path + ".tmp")
    remove_checkpoint(path)  # nothing left to remove, and that is not an error
