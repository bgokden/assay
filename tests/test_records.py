...
def test_parse_criteria_style_record():
    d = {
        ...
        "questions": {
            ...
            "asked": {"type": "noul", "instructions": "Did they ask?", "label": True},
        },
        "_meta": {"source": "suite"},
    }
    r = parse_record(d)
    ...
    assert by_name["asked"].question.type == "bool"  # the other tools' name for it
    assert by_name["asked"].label_index == 0
...
def test_noul_alias_is_read_as_bool():
    ...
    r = parse_record(
        {
            "state": "s",
            "questions": {"q": {"type": "noul", "instructions": "Is it?", "label": True}},
        }
    )
